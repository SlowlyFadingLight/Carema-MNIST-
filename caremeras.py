...
import torch
from config import *
from datasets import neural_net
from torch.autograd import Variable

def get_args():
    parser = argparse.ArgumentParser(
        """Let's write """)
    parser.add_argument("-c", "--color", type=str, choices=["green", "blue", "red"], default="blue",
    ...
    parser.add_argument("-d", "--display", type=int, default=1, help="How long is prediction shown in second(s)")
...
def main(opt):
    ...
    while True:
        ...
        if is_shown:
            A =  str(int(predicts[0]))
            cv2.putText(frame, A, (100, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.5, color_pointer, 5, cv2.LINE_AA)
